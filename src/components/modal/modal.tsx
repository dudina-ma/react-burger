  ...
  return createPortal(
    <>
      ...
      <div className={`${styles.modal} ${paddingClass}`}>
        <header
          className={`${styles.header} ${!title ? styles.header_no_title : ''} ${titleCentered ? styles.header_centered : ''}`}
        >
          {title && (
            <h3
              className={`${titleClassName} m-0 ${titleCentered ? styles.title_centered : ''}`}
            >
              {title}
            </h3>
          )}
